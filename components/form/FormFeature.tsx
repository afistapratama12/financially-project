...
export default function FormFeature({
  ...
  const changeNameDetail = (
    e: React.ChangeEvent<HTMLInputElement>,
    index: number
  ) => {
    const newDetail =
      dataDetail.length > 0 ? [...dataDetail] : [{ name: "", amount: 0 }];
  ...
  return (
    <div>
      <Text paddingBottom={"2"} fontWeight={"semibold"}>
        {name}
      </Text>
        ...
        <>
          {dataDetail.map((detail, index) => (
            <div>
              ...
              <Text color={"red"}>
                {showErrorEmptyDetail(index, detail) &&
          ...
          <Flex gap={"4"}>
            <Button onClick={addDataDetail}>Tambah</Button>

            <Button onClick={changeToNoDetail}>Batal Merinci</Button>
          </Flex>
        </>
      )}
      ...
      {!isDataDetail && (
        <Flex gap={"4"}>
          ...
          {isWithDetail && (
            <Button onClick={changeToDetail}>Ingin Merinci</Button>
          )}
        </Flex>
      )}

      <Text color={"red.500"}>
        {showErrorEmptyNoDetail() && "mohon lengkapi data ini"}
      </Text>
    </div>
  );
}
