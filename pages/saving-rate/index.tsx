...
export default function savingRate() {
  ...
  const [otherIncomeDetail, setOtherIncomeDetail] = useState<
    SavingRateDetail[]
  >([{ name: "", amount: 0 }]);
  const [investmentDetail, setInvestmentDetail] = useState<SavingRateDetail[]>([
    { name: "", amount: 0 },
  ]);
  const [installmentDetail, setInstallmentDetail] = useState<
    SavingRateDetail[]
  >([{ name: "", amount: 0 }]);
  const [expensesDetail, setExpensesDetail] = useState<SavingRateDetail[]>([
    { name: "", amount: 0 },
  ]);
  ...
  const onClickProcessResult = (
    ...
    router.push({
      pathname:
        path.SAVING_RATE_RESULT +
        generateSlug(
          localStorage.getItem(local.VISITOR_FULLNAME) || "",
          getSerial()
        ),
    });
  };
  ...
  const getRangeDescription = () => {
    return state.value.type.savingRateRange === savingRateRange.MONTH
      ? "bulan ini"
      : "dalam setahun";
  };

  return (
    <Container maxW={"1200px"} paddingY={"14"}>
      <VisitorModal isOpen={isOpen} onOpen={onOpen} onClose={onClose} />
      <div>
        <Box paddingBottom={"10"}>
          <Button onClick={() => router.back()}>Kembali</Button>
        </Box>

        <Heading paddingBottom={"10"} fontSize={"2xl"}>
          Form pengisian saving Rate
        </Heading>
        <Card bgColor={"white"} padding={"8"} marginBottom={"5"}>
          <Text paddingBottom={"5"}>
            Berapa lama ingin menghitung saving rate
          </Text>

          <Flex gap={"4"}>
            <Button onClick={() => setSavingRateRange(savingRateRange.MONTH)}>
              Bulan ini
            </Button>
            <Button onClick={() => setSavingRateRange(savingRateRange.YEAR)}>
              Setahun ini
            </Button>
          </Flex>
        </Card>
        ...
        <div>
          <Card bgColor={"white"} padding={"8"} marginBottom={"5"} gap={"4"}>
            <FormFeature
